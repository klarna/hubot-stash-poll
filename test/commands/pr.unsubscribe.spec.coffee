...
describe 'commands | pr | unsubscribe', ->
  ...
  it 'should register a listener', ->
    expect(context.robot.respond.withArgs(/stash pr unsubscribe (.*)/i).calledOnce).to.equal true
  ...
  it 'should unsubscribe the room from the given repo', (done) ->
    ...
    context.robot.adapter.receive new TextMessage(context.user, "#{context.robot.name} stash pr unsubscribe http://mocha.com/")
